    ...
    public class Ship : GameObject
    {
        ...
        public Vector2 AvoidObstacles()
        {
            ...
            foreach (GameObject other in _nearbyShips)
            {
                ...
                float dist = MathF.Max(MathF.Sqrt(distSq), MinAvoidanceDist);
                float sqrtDist = MathF.Sqrt(dist);

                avoidance += _sqrtAvoidanceRange * speed * difference / (dist * sqrtDist);
            }

            float avoidanceSpeedSq = avoidance.LengthSquared();
            if (avoidanceSpeedSq > MaxAvoidanceSpeed * MaxAvoidanceSpeed)
            {
                avoidance *= MaxAvoidanceSpeed / MathF.Sqrt(avoidanceSpeedSq);
            }
        ...
        public Ship FindNearestEnemy(GameTime gameTime)
        {
            ...
            if (nearest == null)
            {
                ...
                if (rawList != null)
                {
                    float bestFallbackDistSq = float.MaxValue;
